...
class TestOrcaMatrixApp(unittest.TestCase):
    ...
    def test_charts_canvas_rendering(self):
        ...
        # Test pareto hover tracking
        hover_points = [item for item in self.app._chart_hover_items if item.get("type") == "point"]
        self.assertGreater(len(hover_points), 0)
        target_pt = hover_points[0]
        ev_pt_hover = MockEvent()
        ev_pt_hover.x = int(target_pt["cx"])
        ev_pt_hover.y = int(target_pt["cy"])
        self.app._on_chart_motion(ev_pt_hover)
        self.assertIn("🎯", self.app.chart_hover_lbl.cget("text"))

        # Verify 3D cube radio button is removed
        self.assertFalse(hasattr(self.app, "chart_3d_radio"))
