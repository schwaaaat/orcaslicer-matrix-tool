# Windows v2.0.3 release guide

Matrix Studio v2.0.3 is distributed as a portable Windows x64 application. No Python runtime, compiler, or repository checkout is required.

## Recommended installation

1. Open the [latest GitHub release](https://github.com/schwaaaat/orcaslicer-matrix-tool/releases/latest).
2. Download `OrcaSlicer-MatrixStudio-v2.0.3-Windows-x64.zip` and `SHA256SUMS.txt`.
...
```text
OrcaSlicer-MatrixStudio-v2.0.3-Windows-x64/
├── orca-slicer.exe
├── OrcaSlicer.dll
├── resources/
├── tools/
...
`OrcaMatrixStudio-v2.0.3-Windows-x64.zip` contains only the companion application. It is useful when updating Matrix Studio independently, but it still requires a running compatible OrcaSlicer build for slicing.
...
In v2.0.3 and newer, OrcaSlicer passes its live API token directly to Matrix Studio when launching it from the menu. On a standalone launch, Matrix Studio first restores a manually saved token from Windows Credential Manager, then falls back to OrcaSlicer's configuration file. The token is shown in the masked API-token field automatically and persists after **Save settings** is pressed.
...
The live-token handoff and corrected credential priority are documented on the [v2.0.3 release page](https://github.com/schwaaaat/orcaslicer-matrix-tool/releases/tag/v2.0.3). Earlier fixes are documented on the [v2.0.2 release page](https://github.com/schwaaaat/orcaslicer-matrix-tool/releases/tag/v2.0.2) and [v2.0.1 release page](https://github.com/schwaaaat/orcaslicer-matrix-tool/releases/tag/v2.0.1).
